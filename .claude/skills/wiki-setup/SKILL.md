...
**Why it is needed:** The vault stores Obsidian connection credentials in `.env`. direnv ensures those values are available in the shell before Claude Code starts, without you having to export them manually each time.

**Check:** Follow [OS detection and app checks](#reference--os-detection-and-app-checks) to check if `direnv` is installed.

- If found: move to sub-step 1a.
...
**Sub-step 1a — direnv.toml config:**
...
- If `load_dotenv = true` is present: confirm and move to sub-step 1b.
...
**Sub-step 1b — Allow direnv in this repo:**
...
- If the current directory is already allowed: confirm and move to Step 2.
...
**Sub-step 3a — Install Obsidian:**

Follow [OS detection and app checks](#reference--os-detection-and-app-checks) to check if Obsidian is installed.

- If installed: confirm and tell the user to open this repo as a vault if not already open (**File → Open folder as vault → select the repo root**). Ask them to confirm the vault is open, then move to sub-step 3b.
...
**Sub-step 3b:** Tell the user to go to **Settings → Community plugins**, disable Safe mode if prompted, click **Browse**, search for **Local REST API**, and install and enable it. Ask them to confirm when done.

**Sub-step 3c:** Tell the user to go to **Settings → Local REST API** and enable **"Enable Non-Encrypted (HTTP) Server"**. Explain that HTTP is required because MCP clients cannot verify the self-signed HTTPS certificate. Ask them to confirm when done.

**Sub-step 3d:** Tell the user to note their **API key** and **port** from **Settings → Local REST API** (default port: `27123`). Remind them that each vault should use its own port to avoid conflicts. Ask them to share the values so you can use them in the next sub-step.

**Sub-step 3e — `.env` setup:**
...
```bash
grep -E "OBSIDIAN_PROTOCOL|OBSIDIAN_HOST|OBSIDIAN_PORT|OBSIDIAN_API_KEY" .env
```

All four should be present. Then tell the user:

- Obsidian must be running with the vault open before starting each Claude Code session.
- Run `/mcp` inside a Claude Code session to confirm `obsidian` shows a **connected** status.
